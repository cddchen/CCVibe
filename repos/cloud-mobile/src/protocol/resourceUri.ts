import { assertOpaqueId, MAX_OPAQUE_ID_BYTES } from './ids';
import type { Brand, ChatId, SessionId } from './ids';
...
function validateSegment(value: string, label: string): void {
  ...
  if (!OPAQUE_SEGMENT.test(value) || !OPAQUE_SEGMENT.test(decoded) || decoded === '.' || decoded === '..') {
...
export function createSessionUri(sessionId: string | SessionId): SessionUri {
...
export function createChatUri(sessionId: string | SessionId, chatId: string | ChatId): ChatUri {
  assertOpaqueId(sessionId, 'sessionId');
  assertOpaqueId(chatId, 'chatId');
...
export function parseResourceUri(value: string): ParsedResource {
  ...
  if (value.startsWith('agent-chat://')) {
    ...
    return Object.freeze({ kind: 'chat', uri, sessionId, chatId });
  }
