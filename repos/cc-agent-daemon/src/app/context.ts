...
export function createAppContext(config: DaemonConfig): AppContext {
  ...
  const sessions = new SessionRegistry(() => createClaudeEngine(), permissions, {
    autoReclaimMs: config.autoReclaimMs,
    maxThreads: config.maxThreads,
  });
  const conversations = new ConversationService(sessions, store);
  return { config, token, store, permissions, sessions, conversations };
}
